...
def get_centers(im):
   ...
   x_cent = np.nansum(im*x[None, :])/np.nansum(im) - 0.5
...
class Centering_AO_system():
    def __init__(self):
        ...
        if MODE_BASIS=='zernike':
            self.modal_norm = np.sqrt(np.array([hp.noll_to_zernike(i)[0] for i in np.arange(2, N_MODES+2)])**(PL_IDX))
            self.modal_norm = 1
        else:
            self.modal_norm = 1.
        self.iteration = 0
    ...
    def step(self, action):
        ...
        self.observation = get_centers(self.image)
        self.strehls.append(self.strehl)
        self.reward = -np.sum(self.observation**2)
        self.ep_reward += self.reward
        self.terminated = (np.sqrt(np.sum(self.observation**2))>0.4)
    ...
    def render(self):
        ...
        if (self.episode == 1 and self.iteration == 1) or not hasattr(self, 'cbar1'):
        ...
        if (self.episode == 1 and self.iteration == 1) or not hasattr(self, 'cbar2'):
        ...
        if (self.episode == 1 and self.iteration == 1) or not hasattr(self, 'cbar3'):
    ...
    def make_dm(self, mode_basis='actuators'):
        if mode_basis == 'actuators':
            ...
            influence_functions = hp.make_zernike_basis(
                N_MODES+1, self.diameter, self.pupil_grid)
            self.deformable_mirror = hp.DeformableMirror(influence_functions)
            self.num_modes = self.deformable_mirror.num_actuators
            self.influence_matrix = np.array(
